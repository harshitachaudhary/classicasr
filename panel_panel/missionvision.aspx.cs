...
public partial class nyksi_panel_Upload_Main_Product : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if(!IsPostBack)
        {
            ...
            if(dt.Rows.Count >0)
            {
                txtmission.Content = dt.Rows[0]["content"].ToString();
                txtvision.Content = dt.Rows[0]["heading1"].ToString();
    ...
    protected void Button1x_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd.Parameters.AddWithValue("@typee", "missionvision");
            cmd.Parameters.AddWithValue("@content", txtmission.Content);
            cmd.Parameters.AddWithValue("@heading1", txtvision.Content);
